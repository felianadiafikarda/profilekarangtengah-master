      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="glass-header sticky top-0 z-20 px-8 py-4 flex justify-between items-center">
          <div>
            ...
            <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Kelola Potensi</h1>
          </div>
          <div className="flex items-center gap-4">
            ...
            <Link href="/admin/potensi/tambah" className="add-btn text-white px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2">
              <span className="text-lg leading-none">+</span> Tambah Potensi
            </Link>
          </div>
        </header>
        ...
        <main className="p-8 flex-1">
          ...
          <div className="grid grid-cols-3 gap-5 mb-8">
            {[
              { label: 'Total Potensi', value: potensi.length, color: 'from-indigo-500 to-violet-500', textC: 'text-indigo-600' },
              { label: 'Kategori Aktif', value: totalKategori, color: 'from-emerald-500 to-teal-500', textC: 'text-emerald-600' },
              { label: 'Data Aktif', value: potensi.length, color: 'from-amber-500 to-orange-400', textC: 'text-amber-600' },
            ].map((stat) => (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <span className="font-bold text-gray-700 text-base">Daftar Potensi</span>
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-gray-50 to-indigo-50 border-b border-gray-100">
                    ...
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">Nama Potensi</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">Kategori</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">Deskripsi</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {potensi.map((p: Potensi, idx: number) => (
                    <tr
                      key={p.id}
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <img
                          src={p.image}
                          alt={p.judulGambar}
                          className="row-img w-12 h-12 object-cover rounded-xl border-2 border-gray-100 shadow-sm"
                        />
                      </td>
                      <td className="px-6 py-4">
                        <div className="font-bold text-gray-800 text-sm">{p.judulGambar}</div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="kategori-badge">{p.sektor}</span>
                      </td>
                      <td className="px-6 py-4 max-w-xs">
                        <p className="text-gray-500 text-xs line-clamp-2">{p.deskripsiGambar}</p>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex gap-2">
                          <Link
                            href={`/admin/potensi/${p.id}/detail`}
