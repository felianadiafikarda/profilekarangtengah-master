    ...
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      <aside className="sidebar-bg w-64 flex flex-col shadow-2xl flex-shrink-0">
        ...
        <nav className="flex-1 p-4 space-y-1">
          ...
          {[
            ...
            { icon: FaLayerGroup, label: "Potensi",   href: "/admin/potensi" },
            { icon: FaUsers,      label: "Users",     href: "/admin/users" },
            { icon: FaCog,        label: "Settings",  href: "/admin/settings" },
          ].map(({ icon: Icon, label, href }) => (
            <Link key={label} href={href} className={`sidebar-link flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold text-indigo-200 hover:text-white ${label === "Potensi" ? "active text-white" : ""}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="glass-header z-20 px-8 py-4 flex justify-between items-center flex-shrink-0">
          <div>
            <div className="text-xs text-indigo-400 font-semibold uppercase tracking-widest mb-0.5">Potensi</div>
            <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Tambah Potensi</h1>
          </div>
          <Link href="/admin/potensi" className="flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-indigo-600 transition-colors">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="flex items-center gap-2 text-xs text-gray-400 font-medium mb-4">
            <Link href="/admin/potensi" className="hover:text-indigo-500 transition-colors">Potensi</Link>
            <span>/</span>
