// app/admin/pemerintahan/perangkat/[id]/edit/page.tsx
import prisma from '@/lib/prisma';
import { notFound } from 'next/navigation';
import Link from 'next/link';
import { FaArrowLeft, FaLayerGroup, FaHome, FaLandmark, FaSeedling, FaBuilding } from 'react-icons/fa';
import EditPerangkatForm from './EditPerangkatForm';
import LogoutButton from '@/components/LogoutButton';
import ProfileModal from '@/components/ProfileModal';

export const dynamic = "force-dynamic";
export default async function EditPerangkat({ params }: { params: { id: string } }) {
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="glass-header z-20 flex-shrink-0 px-8 py-4 flex justify-between items-center">
          <div>
            ...
            <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Edit Perangkat Desa</h1>
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="flex items-center gap-2 text-xs text-gray-400 font-medium mb-6">
            ...
            <span>/</span>
            <span className="text-gray-600">Edit Perangkat</span>
          </div>
          <EditPerangkatForm perangkat={perangkat} />
        </main>
      </div>
    </div>
  );
